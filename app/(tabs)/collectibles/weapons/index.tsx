...
const collectibleCategories = Object.entries(WEAPONS_CATEGORY_TITLES).map(
  ([key, title]) => ({ key, title })
);
...
export default function WeaponsScreen() {
  ...
  const getCategoryCounters = (key: string) => {
    ...
    const filteredWeapons = weapons.filter(filterFn);
    const total = filteredWeapons.length;
    const checked = filteredWeapons.filter(w => selectedWeapons[w.id]).length;
