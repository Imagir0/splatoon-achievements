...
export default function ShoesCategoryScreen() {
  const { shoesCategory } = useLocalSearchParams<{ shoesCategory: string }>();
  ...
  const title = GEARS_CATEGORY_TITLES[shoesCategory  ?? ''] ?? 'Catégorie';

  // Filtrage des vêtements pour la catégorie
  const filteredGears = useMemo(() => {
    const allShoes = GEARS_DATA.shoes;

    return allShoes.filter(
      (gear) =>
        normalizeBrand(gear.brand.name) === shoesCategory 
    );
  }, [shoesCategory ]);
  ...
  return (
    <View style={{ flex: 1, padding: 16 }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const isChecked = isOwned('shoes', item.id);

          return (
            <Pressable
              style={[styles.row, isChecked && styles.rowChecked]}
              onPress={() => toggleGear('shoes', item.id)}
            >
