    ...
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      {categories.map(([key, title]) => {
        ...
        const filtered = useMemo(
          () => GEARS_DATA.heads.filter(g => g.brand.name.toLowerCase() === key.toLowerCase()),
          [key]
        );

        const total = filtered.length;
        const obtained = filtered.filter(g => isOwned('heads', g.id)).length;
        ...
        return (
          <Pressable
            ...
            onPress={() =>
              router.push({
                pathname: '/(tabs)/collectibles/gears/heads/[headsCategory]',
