...
export default function ClothesCategoryScreen() {
  const { headsCategory } = useLocalSearchParams<{ headsCategory: string }>();
  ...
  const title = GEARS_CATEGORY_TITLES[headsCategory ?? ''] ?? 'Catégorie';

  // Filtrage des vêtements pour la catégorie
  const filteredGears = useMemo(() => {
    const allClothes = GEARS_DATA.heads;
        ...
        renderItem={({ item }) => {
          ...
          return (
            <Pressable
              ...
              onPress={() => toggleGear('heads', item.id)}
            >
