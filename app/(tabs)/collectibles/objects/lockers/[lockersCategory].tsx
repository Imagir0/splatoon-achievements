...
const LOCKERS_CATEGORY_TITLES: Record<string, string> = {
  general: 'Casiers',
};

export default function LockersCategoryScreen() {
  const { lockersCategory } =
    useLocalSearchParams<{ lockersCategory: string }>();

  const { isOwned, toggleObject } = useObjects();

  const title =
    LOCKERS_CATEGORY_TITLES[lockersCategory ?? ''] ?? 'Casiers';

  const lockers = useMemo(() => {
    if (!lockersCategory) return [];
